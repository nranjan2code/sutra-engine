...
class SourceReliability:
    ...
    def update_score(self) -> None:
        ...
        if total == 0:
            ...
            score = (
                self.num_correct - self.num_incorrect - (self.num_contradicted * 0.5)
            )
    ...
    def detect_contradictions(
        ...
        for other_id in to_check:
            ...
            contradiction = self._check_contradiction(
                new_concept, new_concept_id, other_concept, other_id
            )
    ...
    def resolve_contradiction(
        ...
        if contradiction.resolved:
            ...
            if contradiction.resolution is None:
                raise ValueError(
                    f"Contradiction {contradiction.id} marked as resolved but has no resolution"
                )
        ...
        logger.info(
            f"Resolved {contradiction} using {strategy.value}: winner={winner[:8]}"
        )
        return winner
    ...
    def _get_source_reliability(self, source: Optional[str]) -> float:
        ...
        if source not in self.source_reliability:
            self.source_reliability[source] = SourceReliability(
                source_id=source, reliability=0.5
            )
        ...
        return {
            ...
            "by_type": {
                conflict_type.value: sum(
                    1 for c in self.contradictions if c.conflict_type == conflict_type
                )
