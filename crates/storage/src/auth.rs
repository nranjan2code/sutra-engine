...
impl Claims {
    ...
    pub fn can_perform(&self, operation: &str) -> bool {
        ...
        match operation {
            ...
            "write" | "learn" | "create" => {
                self.has_role(&Role::Writer) || self.has_role(&Role::Service)
            }
...
impl AuthManager {
    ...
    pub fn new_hmac_with_rate_limit(
        secret: String,
        token_ttl: u64,
    ...
    pub fn new_jwt_hs256_with_rate_limit(
        secret: String,
        token_ttl: u64,
        rate_limit_config: RateLimiterConfig,
    ) -> Self {
    ...
    pub fn from_env() -> Result<Self> {
        let auth_method = std::env::var("SUTRA_AUTH_METHOD").unwrap_or_else(|_| "hmac".to_string());
        ...
        match auth_method.as_str() {
            "hmac" => Ok(Self::new_hmac_with_rate_limit(
                secret,
                token_ttl,
                rate_limit_config,
            )),
            "jwt" | "jwt-hs256" => Ok(Self::new_jwt_hs256_with_rate_limit(
                secret,
                token_ttl,
                rate_limit_config,
            )),
...
mod tests {
    ...
    fn test_hmac_token_generation_and_validation() {
        ...
        let token = manager
            .generate_token("user123", vec![Role::Writer])
            .unwrap();
    ...
    fn test_jwt_token_generation_and_validation() {
        ...
        let token = manager
            .generate_token("service-api", vec![Role::Service])
            .unwrap();
        let claims = manager.validate_token(&token).unwrap();
