...
impl GraphIndex {
    ...
    pub fn insert_concept(
        ...
        temporal.entry(created_at).or_default().push(concept_id);
        drop(temporal);
    ...
    pub fn add_edge(&self, source: ConceptId, target: ConceptId) {
        // Add forward edge
        self.adjacency_index.entry(source).or_default().push(target);

        // Add backward edge (for bidirectional queries)
        self.adjacency_index.entry(target).or_default().push(source);
    ...
    pub fn query_at_time(&self, timestamp_ms: u64) -> Vec<ConceptId> {
        let temporal = self.temporal_index.read();
        temporal.get(&timestamp_ms).cloned().unwrap_or_default()
    }
