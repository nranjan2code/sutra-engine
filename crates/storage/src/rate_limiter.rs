...
impl RateLimiter {
        ...
        // Fast path: Check if bucket exists (read lock)
        {
            ...
            if let Some(bucket) = buckets.get(subject) {
                ...
                let estimated_tokens =
                    bucket.tokens + (elapsed * self.config.requests_per_second as f64);
    ...
    fn cleanup_stale_buckets(&self) {
        ...
        *last_cleanup = Instant::now();
        ...
        log::debug!(
            "🧹 Rate limiter cleanup: {} active buckets remaining",
            buckets.len()
        );
    }
    ...
    pub fn stats(&self) -> RateLimiterStats {
        ...
        RateLimiterStats {
            ...
            throttled_subjects,
            average_tokens: if buckets.is_empty() {
                0.0
            } else {
                total_tokens / buckets.len() as f64
            },
        }
    }
...
mod tests {
    ...
    fn test_basic_rate_limiting() {
        ...
        for i in 0..10 {
            assert!(
                limiter.check_rate_limit(subject).is_ok(),
        ...
        assert!(
            limiter.check_rate_limit(subject).is_err(),
