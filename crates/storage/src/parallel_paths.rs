                ...
                for &neighbor in &node.neighbors {
                    if let std::collections::hash_map::Entry::Vacant(e) = visited.entry(neighbor) {
                        ...
                        if neighbor == end {
                            ...
                            let confidence =
                                PathResult::calculate_confidence(path_length, self.decay_factor);
...
mod tests {
    ...
    fn test_parallel_multiple_paths() {
        ...
        node1.add_edge(
            id2,
            AssociationRecord::new(id1, id2, AssociationType::Semantic, 0.9),
        );
        node1.add_edge(
            id3,
            AssociationRecord::new(id1, id3, AssociationType::Semantic, 0.8),
        );

        let mut node2 = ConceptNode::new(id2, vec![2], None, 1.0, 0.9, 1000);
        node2.add_edge(
            id4,
            AssociationRecord::new(id2, id4, AssociationType::Semantic, 0.9),
        );

        let mut node3 = ConceptNode::new(id3, vec![3], None, 1.0, 0.9, 1000);
        node3.add_edge(
            id4,
            AssociationRecord::new(id3, id4, AssociationType::Semantic, 0.8),
        );
    ...
    fn test_parallel_no_path() {
        ...
        snapshot
            .concepts
            .insert(id1, ConceptNode::new(id1, vec![1], None, 1.0, 0.9, 1000));
        snapshot
            .concepts
            .insert(id2, ConceptNode::new(id2, vec![2], None, 1.0, 0.9, 1000));
    ...
    fn test_best_path() {
        ...
        node1.add_edge(
            id2,
            AssociationRecord::new(id1, id2, AssociationType::Semantic, 0.8),
        );

        let mut node2 = ConceptNode::new(id2, vec![2], None, 1.0, 0.9, 1000);
        node2.add_edge(
            id3,
            AssociationRecord::new(id2, id3, AssociationType::Semantic, 0.8),
        );
        ...
        let finder = ParallelPathFinder::default();
        let path = finder
            .find_best_path(Arc::new(snapshot), id1, id3, 10)
            .unwrap();
