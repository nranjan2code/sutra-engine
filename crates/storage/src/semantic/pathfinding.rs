...
impl SemanticPathFinder {
    ...
    pub fn find_contradictions(
        ...
        for concept in snapshot.all_concepts() {
            if let Some(ref semantic) = concept.semantic {
                if semantic.semantic_type == SemanticType::Rule && semantic.domain_context == domain
                {
...
mod tests {
    ...
    fn test_semantic_pathfinding_with_filter() {
        ...
        let filter = SemanticFilter::new().with_domain(DomainContext::Medical);

        let paths = pathfinder.find_paths_filtered(Arc::new(snapshot), id1, id3, &filter);
