...
use std::sync::Arc;
use sutra_storage::{ConcurrentConfig, ConcurrentMemory, LearningStorage, NamespaceManager};
...
async fn test_multi_namespace_isolation() {
    ...
    let storage1 = manager
        .create_namespace(ns1, ConcurrentConfig::default())
        .unwrap();
    let storage2 = manager
        .create_namespace(ns2, ConcurrentConfig::default())
        .unwrap();
