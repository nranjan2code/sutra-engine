...
use sutra_storage::{
    concurrent_memory::{ConcurrentMemory, ConcurrentConfig},
    types::{ConceptId, AssociationType},
...
fn create_diamond_graph(memory: &ConcurrentMemory, layers: usize) -> (ConceptId, ConceptId) {
    ...
    memory.learn_concept(start, b"start".to_vec(), None, 1.0, 0.9).unwrap();
    memory.learn_concept(end, b"end".to_vec(), None, 1.0, 0.9).unwrap();
    ...
    for layer in 0..layers {
        ...
        for (i, &parent) in current_layer.iter().enumerate() {
            ...
            for child_idx in 0..2 {
                ...
                memory.learn_concept(child_id, format!("layer{}_node{}", layer, node_id).as_bytes().to_vec(), None, 1.0, 0.9).unwrap();
                memory.learn_association(parent, child_id, AssociationType::Semantic, 0.8).unwrap();
    ...
    for &node in &current_layer {
        memory.learn_association(node, end, AssociationType::Semantic, 0.9).unwrap();
    }
